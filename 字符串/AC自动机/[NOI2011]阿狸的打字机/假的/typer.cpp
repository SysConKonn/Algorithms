...
void rebuild () {
  ...
  for (int i = 0;i < len_; ++i) {
    ...
    if (t[i] == 'P') {
      ++pt;
      for (int i = head[x[pt]]; i; i = nxt[i]) {
	int now = to[i];
	totans[x[i]] = bit_find (r[now]) - bit_find(l[now]);
      }
    }
    else if (t[i] == 'B') {
      bit_add(u, -1);
...
int main () {
  ...
  int _len_ = strlen(t);
  int summ = 0;
  for (int i = 0;i < _len_; ++i) if (t[i] == 'P') summ++;
  summ++;
  int ssum = 0;
  for (int i = 1;i <= summ; ++i) {
    if (_book_[i]) y[++ssum] = i;
  }
  /*
  for (int i = 1;i <= ssum; ++i) printf("%d ", x[i]);
  printf("\n");
  */
  get_dfs_order(0, 0);
  /*
